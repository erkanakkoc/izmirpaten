...
import { LayoutDashboard, Users, CalendarDays, BookOpen, LogOut, Zap, ChevronRight, Menu, X } from 'lucide-react'
...
const navItems = [
  ...
  { href: '/trainer/lessons', label: 'Dersler', icon: BookOpen },
  { href: '/trainer/students', label: 'Öğrencilerim', icon: Users },
  { href: '/trainer/availability', label: 'Müsaitlik', icon: CalendarDays },
]

function NavContent({ trainerName, onClose }: { trainerName: string; onClose?: () => void }) {
  ...
  return (
    <div className="w-64 bg-[#1B2A4A] text-white flex flex-col h-full">
      <div className="px-6 py-4 border-b border-white/10 flex flex-col items-center gap-1.5">
        {onClose && (
        ...
        <div className="text-center">
          <div className="font-extrabold text-sm">{trainerName}</div>
          <div className="text-xs text-blue-300">Eğitmen Paneli</div>
        </div>
      </div>
...
export default function TrainerSidebar({ trainerName }: { trainerName: string }) {
  const [mobileOpen, setMobileOpen] = useState(false)
  return (
    <>
      <aside className="hidden md:flex flex-shrink-0 h-screen sticky top-0">
        <NavContent trainerName={trainerName} />
      </aside>
      <div className="md:hidden fixed top-0 left-0 right-0 z-40 bg-[#1B2A4A] text-white flex items-center gap-3 px-4 h-14 border-b border-white/10">
        ...
        <span className="font-extrabold text-sm">{trainerName}</span>
      </div>
      {mobileOpen && (
        <div className="md:hidden fixed inset-0 z-50 flex">
          <div className="absolute inset-0 bg-black/60" onClick={() => setMobileOpen(false)} />
          <div className="relative z-10 h-full">
            <NavContent trainerName={trainerName} onClose={() => setMobileOpen(false)} />
          </div>
        </div>
      )}
    </>
