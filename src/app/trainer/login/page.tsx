...
function LoginForm() {
  ...
  const handleReset = async (e: React.FormEvent) => {
    ...
    const { error: resetError } = await supabase.auth.resetPasswordForEmail(resetEmail, {
      redirectTo: `${siteUrl}/trainer/reset-password`,
    })
    setResetLoading(false)
    if (resetError) {
      toast.error('Sıfırlama maili gönderilemedi: ' + resetError.message)
      return
    }
    setResetSent(true)
  }
            ...
            <div className="bg-orange-50 border border-orange-200 rounded-xl p-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <input
                  ...
                  className="w-20 px-3 py-2 rounded-xl border border-gray-200 text-sm focus:outline-none focus:border-[#FF6B35]"
                  placeholder="?"
                />
              ...
              {captchaVerified && (
                <p className="text-xs text-green-600 font-semibold mt-2">✅ Doğrulandı</p>
              )}
            </div>
          )}
          {needsCaptcha && captchaVerified && (
        ...
        <div className="bg-white rounded-3xl p-8 shadow-2xl">
          {!resetSent ? (
            <>
              ...
              <p className="text-sm text-gray-500 mb-5">E-posta adresinize sıfırlama bağlantısı göndereceğiz.</p>
